...
local model = nn.memory.PriorityQueueTransducer(
    inputVocab.size, outputVocab.size, opt.dim_size, opt.num_layers, 
    optimState) 
...
for chunk=1,#trainDataChunks do
    ...
    for batch in data.batches(testData, opt.batch_size, use_GPU) do 
        ...
        local err
        if chunk < 3 then
            err = model:lossCoupledLSTM(
                batch.encoderInput, batch.decoderInput, batch.decoderOutput)
        else
            err = model:lossPriorityQueue(
                batch.encoderInput, batch.decoderInput, batch.decoderOutput)
        end
