...
Options:
   ...
   --couple-iter     (default 50)    Iteration at which to couple enc/dec
   --num-queues      (default 1)     Number of priority queue structures
   --read-bias       (default 0)     Read bias init
   --forget-bias     (default 0)     Forget bias init
...
local model = nn.memory.PriorityQueueTransducer(inputVocab.size, 
    outputVocab.size, opt.dim_size, opt.num_queues, opt.num_layers, optimState,
    opt.read_bias, opt.forget_bias)

if use_GPU then model:cuda() end


model.coupledLSTM:get(2):decouple()
--model.coupledLSTM:decouple()
...
for chunk=1,#trainDataChunks do
    ...
    if chunk == opt.couple_iter then 
        print("\n({[ Coupling LSTM ]})\n")
        model.coupledLSTM:get(2):couple()
    end
    ...
    for batch in data.batches(trainChunk, opt.batch_size, use_GPU) do 
        ...
        local err = model:trainPriorityQueue(
    ...
    for batch in data.batches(testData, opt.batch_size, use_GPU) do 
        ...
        local err = model:lossPriorityQueue(
            batch.encoderInput, batch.decoderInput, batch.decoderOutput)
    ...
    out:write(opt.seed .. "\t" .. opt.learning_rate .. "\t" .. opt.dim_size .. 
        ...
        opt.couple_iter .. "\t" .. opt.num_queues .. "\t" .. opt.read_bias
        .. "\t" .. opt.forget_bias .. "\t" ..
